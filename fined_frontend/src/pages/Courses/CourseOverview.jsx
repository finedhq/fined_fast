    ...
    <div className="min-h-screen pb-5 bg-gray-100 overflow-x-hidden font-inter text-[#1e1e1e]">
      {loading ? (
        ...
        <div className="py-5 px-4 sm:py-10 bg-gray-100 min-h-screen">
          <div className="bg-indigo-700 text-white rounded-2xl overflow-hidden mb-8 w-full sm:max-w-3xl sm:mx-auto shadow-md transition-shadow hover:shadow-lg">
            ...
            {course.length > 0 && (
              <div className="px-6 py-3 bg-indigo-800/50">
                <div className="text-xs font-bold uppercase tracking-wider text-indigo-200">Module 1</div>
                <div className="font-semibold text-lg">{course[0]?.moduleTitle}</div>
              </div>
            )}
          </div>

          {course.map((module, index) => (
            <div key={index} className="mb-20 mx-auto max-w-3xl relative">
              {index !== 0 && (
                <div className="bg-indigo-700 text-white px-6 py-4 rounded-2xl font-medium text-left w-full shadow-md hover:shadow-lg transition-shadow mb-8">
                  <div className="text-xs font-bold uppercase tracking-wider text-indigo-200">Module {index + 1}</div>
                  <div className="font-bold text-lg">{module.moduleTitle}</div>
                </div>
              )}

              <div className="mt-10 flex flex-col items-center gap-2 px-4 sm:px-0">
                  ...
                  return (
                    <div key={i} className={`w-full flex flex-col ${i % 2 === 0 ? "items-start" : "items-end"}`}>
                      <div className={`flex flex-col items-center w-1/4 sm:w-1/6 ${i % 2 === 0 ? 'ml-4 sm:ml-[165px]' : 'mr-4 sm:mr-[165px]'} relative z-10`}>
                        <button
                          onClick={() => {
                            if (isClickable) {
                              ...
                              navigate(`/courses/course/${courseId}/module/${module.moduleId}/card/${card.card_id}`);
                            } else {
                        ...
                        <p className="text-center w-32 sm:w-64 mt-2 font-bold text-[11px] sm:text-[14px] text-gray-800 leading-tight">
                          {card.title}
                        </p>
                      </div>
          ...
          {course.length === 0 && !loading && (
             <div className="text-center py-20 bg-white rounded-2xl shadow-sm border border-gray-200 max-w-3xl mx-auto">
